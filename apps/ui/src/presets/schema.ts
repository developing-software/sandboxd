...
export function parsePresetDoc(name: string, doc: unknown): PresetDoc {
  ...
  if (doc.cmd !== undefined && doc.cmd !== null) {
    if (
      !Array.isArray(doc.cmd) ||
      !doc.cmd.length ||
      !doc.cmd.every((c) => typeof c === 'string')
    )
      throw new Error('cmd must be a non-empty list of strings')
    cmd = doc.cmd as string[]
  }

  let idle: number | null = null
  if (doc.idle_timeout_s !== undefined && doc.idle_timeout_s !== null) {
    if (!Number.isInteger(doc.idle_timeout_s) || (doc.idle_timeout_s as number) < MIN_IDLE_S)
      throw new Error(`idle_timeout_s must be an integer >= ${MIN_IDLE_S}`)
  ...
  if (doc.preview !== undefined && doc.preview !== null) {
    if (!Json.isObj(doc.preview)) throw new Error('preview must be a map')
    const { port, port_field, ...rest } = doc.preview
    if (Object.keys(rest).length)
      throw new Error(`preview: unknown key "${Object.keys(rest)[0]}"`)
    if (port !== undefined && port_field !== undefined)
      throw new Error('preview: give either port or port_field')
    if (port_field !== undefined) {
      ...
      if (f.type !== 'int')
        throw new Error(`preview.port_field "${f.name}" must be an int field`)
      preview = { port: typeof f.default === 'number' ? f.default : null, port_field: f.name }
    } else if (port !== undefined) {
      if (!Number.isInteger(port) || (port as number) < 1 || (port as number) > 65535)
        throw new Error('preview.port must be an integer in 1..65535')
  ...
  if (doc.claims_when !== undefined && doc.claims_when !== null) {
    if (!Json.isObj(doc.claims_when)) throw new Error('claims_when must be a map')
    const { present, ...rest } = doc.claims_when
    if (Object.keys(rest).length)
      throw new Error(`claims_when: unknown key "${Object.keys(rest)[0]}"`)
    claims =
      typeof present === 'string'
        ? [present]
        : Array.isArray(present) && present.every((p) => typeof p === 'string')
          ? (present as string[])
          : []
    if (!claims.length)
      throw new Error('claims_when.present must be a field name or a list of them')
...
function parseFields(raw: unknown): FieldSpec[] {
  ...
  if (!Json.isObj(raw)) throw new Error('fields must be a map')
  const out: FieldSpec[] = []
  const envs = new Map<string, string>()
  for (const [path, spec] of Object.entries(raw)) {
    const at = `fields.${path}`
    if (!PATH_RE.test(path))
      throw new Error(`${at}: field names are lowercase identifiers, dotted for nesting`)
    if (!Json.isObj(spec)) throw new Error(`${at} must be a map`)
    for (const k of Object.keys(spec))
      if (!FIELD_KEYS.has(k)) throw new Error(`${at}: unknown key "${k}"`)
    const type = spec.type as FieldType
    if (!TYPES.includes(type)) throw new Error(`${at}.type must be one of ${TYPES.join(', ')}`)
    const env =
      spec.env === undefined ? path.toUpperCase().replace(/[^A-Z0-9_]/g, '_') : spec.env
    if (typeof env !== 'string' || !ENV_RE.test(env))
      throw new Error(`${at}.env must be an environment variable name`)
    if (RESERVED_ENV.has(env)) throw new Error(`${at}.env "${env}" is reserved`)
    if (envs.has(env))
      throw new Error(`${at}.env "${env}" is already used by fields.${envs.get(env)}`)
    envs.set(env, path)
    const f: FieldSpec = {
      name: path,
      type,
      env,
      required: bool(at, 'required', spec.required),
      secret: bool(at, 'secret', spec.secret),
    }
    ...
    if (spec.multiline !== undefined) {
      if (type !== 'string') throw new Error(`${at}.multiline only applies to string fields`)
      f.multiline = bool(at, 'multiline', spec.multiline)
    }
    if (type === 'enum') {
      if (
        ...
        throw new Error(`${at}.values must be a non-empty list of strings`)
      f.values = spec.values as string[]
    } else if (spec.values !== undefined)
      throw new Error(`${at}.values only applies to enum fields`)
    if (type === 'int') {
      if (spec.min !== undefined) {
        if (!Number.isInteger(spec.min)) throw new Error(`${at}.min must be an integer`)
        f.min = spec.min as number
      }
      if (spec.max !== undefined) {
        if (!Number.isInteger(spec.max)) throw new Error(`${at}.max must be an integer`)
        f.max = spec.max as number
      }
      if (f.min !== undefined && f.max !== undefined && f.min > f.max)
        throw new Error(`${at}: min > max`)
    } else if (spec.min !== undefined || spec.max !== undefined)
      throw new Error(`${at}.min/max only apply to int fields`)
...
function checkDefault(at: string, f: FieldSpec, v: unknown): string | number | boolean {
  switch (f.type) {
    case 'string':
      if (typeof v !== 'string') throw new Error(`${at}.default must be a string`)
      return v
    case 'url':
      if (typeof v !== 'string' || !/^https?:\/\//.test(v))
        throw new Error(`${at}.default must be an http(s) url`)
      return v.replace(/\/+$/, '')
    case 'bool':
      if (typeof v !== 'boolean') throw new Error(`${at}.default must be a boolean`)
      return v
    case 'enum':
      if (typeof v !== 'string' || !f.values!.includes(v))
        throw new Error(`${at}.default must be one of ${f.values!.join(', ')}`)
      return v
    case 'int':
      if (
        !Number.isInteger(v) ||
        (f.min !== undefined && (v as number) < f.min) ||
        (f.max !== undefined && (v as number) > f.max)
      )
        throw new Error(
          `${at}.default must be an integer in ${f.min ?? '-inf'}..${f.max ?? 'inf'}`,
        )
      return v as number
  }
}
