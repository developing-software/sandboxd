...
import { EMPTY_CATALOG } from '../src/services'
import { parseCompose } from '../src/compose'

const env = {
  SANDBOXD_SERVICE_TOKEN: 't',
  LLM_API_KEY: 'k',
  LLM_BASE_URL: 'http://gw/',
  SANDBOXD_SANDBOX_ENV_HTTP_PROXY: 'http://p',
}
...
test('config: sandbox env from LLM_* shorthands and SANDBOXD_SANDBOX_ENV_* prefix; presets dir', () => {
  const cfg = loadConfig(env)
  expect(cfg.sandboxEnv).toEqual({
    LLM_API_KEY: 'k',
    LLM_BASE_URL: 'http://gw',
    HTTP_PROXY: 'http://p',
  })
  expect(cfg.presetsDir).toBe(resolve(import.meta.dir, '../../../presets'))
  expect(cfg.defaultImage).toBe('sandboxd-coding-agent:latest')
  expect(
    loadConfig({ ...env, SANDBOXD_PRESETS_DIR: '/etc/sandboxd/presets' }).presetsDir,
  ).toBe('/etc/sandboxd/presets')
})
...
test('shipped presets: one folder each, one image each, catalog from services.yaml', () => {
  ...
  expect(loaded.catalog.list()[0]).toMatchObject({
    name: 'postgres',
    ready: { port: 5432, timeout_s: 90 },
    sandbox_env: { PGHOST: 'postgres' },
  })
...
test('coding-agent: fields expand to entry.sh env; secrets split out; absent fields emit nothing', () => {
  ...
  // AGENT / MODEL / BRANCH are defaulted by entry.sh, so an operator's SANDBOXD_SANDBOX_ENV_* survives (scheduler drops
  // operator keys that the session env also sets).
  expect(preset('coding-agent').expand('s', { repo: 'r' }).env).toEqual({ REPO: 'r' })
  expect(
    preset('coding-agent').expand('s', { repo: 'r', agent: 'codex', model: 'gpt-5' }).env,
...
test('coding-agent: validation messages', () => {
  ...
  expect(() => c.expand('s', { repo: 'r', agent: 'vim' })).toThrow(
    /agent must be one of claude, codex, opencode, shell/,
  )
  expect(() => c.expand('s', { repo: 'r', llm: { base_url: 'ftp://x' } })).toThrow(
...
test('vscode: editor only, previewed on the port field', () => {
  ...
  expect(v.expand('s', { repo: 'https://x/r.git', port: 3000 }).env).toEqual({
    REPO: 'https://x/r.git',
    VSCODE_PORT: '3000',
  })
})
...
test('preset from doc: bool/int coercion, nested paths, url normalisation', () => {
  ...
  expect(p.expand('s', { a: { b: 3 }, flag: false, url: 'https://h/x//' }).env).toEqual({
    A_B: '3',
    FLAG: 'false',
    URL: 'https://h/x',
    S: 'dflt',
...
test('preset services: catalog defaults come with their sandbox env; unknown catalog names fail at boot', () => {
  const catalog = new ComposeCatalog(
    parseCompose({
      services: {
        pg: {
          image: 'postgres',
  ...
  expect(s.decls.map((d) => [d.name, d.image, d.env])).toEqual([
    ['pg', 'postgres', {}],
    ['db', 'postgres', { A: '1' }],
  ])
  ...
  expect(() =>
    presetFromDoc(parsePresetDoc('x', doc({ services: ['redis'] })), EMPTY_CATALOG),
...
test('loader: missing dir and empty dir are boot errors naming the path', () => {
  ...
  expect(() => loadPresetDir(resolve(import.meta.dir, '../src/preview'))).toThrow(
    /no presets found in .*src\/preview/,
  )
})
