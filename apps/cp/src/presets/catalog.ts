...
import type { CatalogEntry, ServiceCatalog, ValidatedServices } from '../services'
...
export class ComposeCatalog implements ServiceCatalog {
  ...
  constructor(services?: ValidatedServices) {
    for (const decl of services?.decls ?? []) {
      this.entries.set(decl.name, {
        decl,
        secret_env: services!.secrets[decl.name] ?? {},
        sandbox_env: services!.sandbox_env[decl.name] ?? {},
      })
    }
  }
  ...
  list(): CatalogView[] {
    return [...this.entries.values()].map(({ decl, sandbox_env }) => ({
      name: decl.name,
      image: decl.image,
      cmd: decl.cmd,
      ready: decl.ready,
