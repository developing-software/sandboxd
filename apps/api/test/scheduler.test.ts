...
class FakeHub implements HostPlacement {
  ...
  createSession(hostId: string, spec: Msg.Spec) {
    ...
    if (!c) return false
    c.running += 1
    this.created.push({ hostId, spec })
    return true
  }
  destroySession(_h: string, sid: string) {
...
function setup(sandboxEnv: Record<string, string> = {}) {
  ...
  let seq = 0
  const session = (id: string, env: Record<string, string> = {}) =>
    store.insertSession({
      id,
      owner_id: 'o',
      image: 'img',
...
test('places on the host with most free slots and forwards secrets once', () => {
  ...
  expect(
    store.db
      ...
      .all()
      .some((r: any) => JSON.stringify(r).includes('GIT_TOKEN')),
  ).toBe(false)
})
...
test('reconciles on host reconnect: unreported sessions are lost', () => {
  const { store, sched, host, session } = setup()
  host('h1', 5)
  sched.submit(session('s_1'), {})
  sched.submit(session('s_2'), {})
  sched.sessionStarted('s_1')
  sched.sessionStarted('s_2')
...
test('env precedence: session secret_env > session env > operator sandboxEnv; operator env never persisted', () => {
  ...
  host('h1', 1)
  sched.submit(session('s_1', { LLM_BASE_URL: 'http://session' }), {
    LLM_API_KEY: 'session-key',
  })
  const spec = hub.created[0]!.spec
  expect({ ...spec.env, ...spec.secret_env }).toEqual({
    LLM_BASE_URL: 'http://session',
    LLM_API_KEY: 'session-key',
    HTTP_PROXY: 'http://proxy',
  })
  expect(spec.env).toEqual({ LLM_BASE_URL: 'http://session' })
  expect(JSON.stringify(store.db.query('SELECT * FROM sessions').all())).not.toContain(
    'op-key',
  )
