...
test('byte-at-a-time feeding and several frames per read', () => {
  const a = encodeWsFrame(OP.TEXT, enc('hello'), true),
    b = encodeWsFrame(OP.PING, enc('p'), false),
    c = encodeWsFrame(OP.TEXT, enc('world'), true)
  const all = new Uint8Array([...a, ...b, ...c])
  const p = new WsFrameParser()
  const got: { opcode: number; s: string }[] = []
  for (const byte of all)
    for (const m of p.feed(new Uint8Array([byte])))
      got.push({ opcode: m.opcode, s: dec(m.data) })
  expect(got).toEqual([
    { opcode: OP.TEXT, s: 'hello' },
    { opcode: OP.PING, s: 'p' },
    { opcode: OP.TEXT, s: 'world' },
  ])
  expect(new WsFrameParser().feed(all).map((m) => dec(m.data))).toEqual([
    'hello',
    'p',
    'world',
  ])
...
test('protocol violations and oversize frames throw', () => {
  expect(() => new WsFrameParser().feed(encodeWsFrame(OP.CONT, enc('x'), false))).toThrow(
    /continuation/,
  )
  const big = encodeWsFrame(OP.BINARY, new Uint8Array(200), false)
  expect(() => new WsFrameParser(100).feed(big)).toThrow(/too large/)
  const frag = encodeWsFrame(OP.BINARY, new Uint8Array(60), false)
  frag[0] = OP.BINARY
  const p = new WsFrameParser(100)
  p.feed(frag)
  expect(() => p.feed(encodeWsFrame(OP.CONT, new Uint8Array(60), false))).toThrow(
    /message too large/,
  )
})
