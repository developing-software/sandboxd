...
import { HttpError, unauthorized } from '@sandboxd/core/errors'
...
import { serviceRoutes, type Env } from './handler/service'
...
const bearer =
  ...
  async (c, next) => {
    ...
    if (c.req.header('authorization') !== `Bearer ${token}`) throw unauthorized()
    return next()
  }
...
export function createRoutes(deps: Deps) {
  return (
    new Hono<Env>({ strict: false })
      ...
      // Every thrown value leaves as the same shape. HttpError carries its status; anything
