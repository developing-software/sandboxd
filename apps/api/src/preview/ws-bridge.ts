...
export type PreviewUpgrader = {
  upgrade(
    req: Request,
    opts: { data: PreviewWsData; headers?: Record<string, string> },
  ): boolean
}
...
export class PreviewBridge {
  ...
  onUpstreamData(d: Uint8Array) {
    let msgs
    try {
      msgs = this.frames.feed(d)
    } catch (e) {
      this.fail(1002, String(e))
      return
    }
  ...
  onUpstreamClose() {
    ...
    this.run(() => {
      try {
        this.ws?.close(1001, 'upstream closed')
      } catch {}
    })
  }
  ...
  onBrowserMessage(msg: string | Buffer) {
    ...
    this.conn.write(
      encodeWsFrame(typeof msg === 'string' ? OP.TEXT : OP.BINARY, payload, true),
    )
  }
  ...
  onBrowserClose(code: number, reason: string) {
    ...
    try {
      this.conn.write(
        encodeWsFrame(OP.CLOSE, encodeClose(validCloseCode(code) ? code : 1000, reason), true),
      )
    } catch {}
    this.conn.close()
  }
  ...
  private deliver(m: { opcode: number; data: Uint8Array }) {
    ...
    switch (m.opcode) {
      case OP.TEXT:
        ws.send(new TextDecoder().decode(m.data))
        break
      case OP.BINARY:
        ws.send(m.data)
        break
      case OP.PING:
        if (!this.upstreamClosed) this.conn.write(encodeWsFrame(OP.PONG, m.data, true))
        break
      case OP.PONG:
        break
      case OP.CLOSE: {
        ...
        if (!this.upstreamClosed) {
          this.upstreamClosed = true
          try {
            this.conn.write(encodeWsFrame(OP.CLOSE, m.data, true))
          } catch {}
          this.conn.close()
        }
        try {
          ws.close(validCloseCode(code) ? code : 1000, reason)
        } catch {}
        break
      }
    }
...
function validCloseCode(c: number) {
...
export async function proxyWebSocket(
  ...
  try {
    const onClose = () => {
      if (bridge) bridge.onUpstreamClose()
    ...
    res = await sendHead(
      ...
      dial,
      (d) => {
        if (bridge) bridge.onUpstreamData(d)
        else early.push(d)
      },
      onClose,
      onClose,
    ...
    return new Response(`could not reach port ${port} in the sandbox: ${String(e)}`, {
      status: 502,
    })
  }
  ...
  if (res.status !== 101) {
    ...
    res.conn.close()
    return new Response(`upstream refused websocket upgrade (${res.status})`, {
      status: res.status,
      headers: passHeaders(res.headers),
    })
  }
  ...
  const headers =
    acceptedProto && req.headers.has('sec-websocket-protocol')
      ? { 'sec-websocket-protocol': acceptedProto }
      : undefined
  if (
    !server.upgrade(req, {
      data: { kind: 'preview', bridge },
      ...(headers ? { headers } : {}),
    })
  ) {
    res.conn.close()
