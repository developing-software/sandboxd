...
export interface Placement {
  pick(candidates: Candidate[]): string | null
}
...
export class Scheduler implements HubEvents {
  /** `secret_env` per queued session; memory-only until placement. */
  ...
  constructor(
    private store: Store,
    private hub: HostPlacement,
    private sandboxEnv: Record<string, string> = {},
    private placement: Placement = mostFreeSlots,
  ) {
  ...
  cancel(s: Session) {
    this.secrets.delete(s.id)
    if (s.status === 'queued') {
      this.store.markEnded(s.id, 'closed')
      return
    }
    if (s.host_id) this.hub.destroySession(s.host_id, s.id)
    if (!s.host_id || !this.hub.isOnline(s.host_id))
      this.store.markEnded(s.id, 'closed', 'host offline at close')
  }
    ...
    for (const s of this.store.activeSessions()) {
      if (s.unknown_since !== null && s.unknown_since < cutoff) {
        this.store.markEnded(
          s.id,
          'lost',
          'host did not report this session after control plane restart',
        )
      }
    }
  }
