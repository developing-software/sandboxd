...
function setup(sandboxEnv: Record<string, string> = {}) {
  ...
  const sched = new Scheduler(store, hub as unknown as HostHub, sandboxEnv);
  ...
  const session = (id: string, env: Record<string, string> = {}) => store.insertSession({
    id, owner_id: "o", preset: "custom", image: "img", cmd: null, env: JSON.stringify(env), idle_timeout_s: 60,
...
test("cmd round-trips as JSON; null means daemon default", () => {
  ...
  host("h1", 2);
  sched.submit(store.insertSession({ id: "s_c", owner_id: "o", preset: "custom", image: "img", cmd: JSON.stringify(["python3", "-m", "http.server"]), env: "{}", idle_timeout_s: 60, created_at: 1 }), {});
