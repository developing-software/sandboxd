...
import { HostConn, type Transport } from "../src/cp/hosts/conn.ts";
import { decodeFrame } from "../src/protocol/framing.ts";
...
class FakeTransport implements Transport {
  ...
  get msgs() { return this.sent.filter((m): m is string => typeof m === "string").map((m) => JSON.parse(m)); }
  get frames() { return this.sent.filter((m): m is Uint8Array => typeof m !== "string").map(decodeFrame); }
}

const setup = () => { const t = new FakeTransport(); return { t, c: new HostConn("h1", t, 0, 4) }; };

test("pty: odd stream ids, frames tagged with the stream, replay/close routed by id", () => {
  const { t, c } = setup();
  const got: string[] = [];
  let replayed = 0, closed = 0;
  const a = c.openPty("s_1", { cols: 80, rows: 24 }, { onData: (d) => got.push(new TextDecoder().decode(d)), onReplay: () => replayed++, onClose: () => closed++ });
  ...
  expect(t.msgs.at(-1)).toEqual({ type: "pty.resize", sid: "s_1", size: { cols: 100, rows: 30 } });
...
test("port: dial resolves on port.open and rejects on port.error; closeAll notifies everyone", async () => {
  ...
  expect(t.msgs.map((m) => [m.type, m.port, m.stream])).toEqual([["port.dial", 3000, 1], ["port.dial", 3001, 3]]);
...
test("createSession counts optimistically until the next heartbeat", () => {
  const { t, c } = setup();
  c.createSession({ sid: "s", image: "i", cmd: null, idle_timeout_s: 60, env: {}, secret_env: {}, services: [] });
