...
class FakeHub implements PortDialer {
  ...
  async dialPort(_h: string, _sid: string, port: number, sub: { onData(d: Uint8Array): void; onClose(): void }) {
    const sock = await Bun.connect({
      ...
      socket: {
        data(_s, d) { sub.onData(new Uint8Array(d)); },
        close() { sub.onClose(); },
        error() { sub.onClose(); },
      },
    });
    return { write(d: Uint8Array) { sock.write(d); }, close() { sock.end(); } };
  }
}
...
beforeAll(() => {
  upstream = Bun.serve<{ proto: string | null }>({
    ...
    async fetch(req, server) {
      ...
      if (req.headers.get("upgrade") === "websocket") {
        ...
        const ok = server.upgrade(req, { data: { proto }, ...(proto ? { headers: { "sec-websocket-protocol": proto } } : {}) });
        return ok ? undefined : new Response("upgrade failed", { status: 500 });
      }
      if (req.method === "POST") return new Response(`posted:${url.pathname}:` + await req.text());
      return new Response(`plain:${url.pathname}`);
    },
    websocket: {
      open(ws) { ws.send(`hello:${ws.data.proto ?? "none"}`); },
      message(ws, m) {
        if (typeof m === "string") { if (m === "close-me") ws.close(4001, "server says bye"); else ws.send("echo:" + m); }
        else ws.send(m);
      },
    },
  });
  ...
  cp = Bun.serve<PreviewWsData>({
    ...
    websocket: {
      open(ws: ServerWebSocket<PreviewWsData>) { ws.data.bridge.attach(ws); },
      message(ws, m) { ws.data.bridge.onBrowserMessage(m); },
      close(ws, code, reason) { ws.data.bridge.onBrowserClose(code, reason); },
    },
  });
});
afterAll(() => { cp.stop(true); upstream.stop(true); });
...
function connect(path: string, protocols?: string[]) {
  const ws = new WebSocket(`ws://127.0.0.1:${cp.port}${path}`, { headers: headers(), protocols } as any);
  const events: (string | Uint8Array | { closed: [number, string] })[] = [];
  const waiters: ((e: unknown) => void)[] = [];
  const push = (e: (typeof events)[number]) => { const w = waiters.shift(); w ? w(e) : events.push(e); };
  ws.binaryType = "arraybuffer";
  ws.onmessage = (ev) => push(typeof ev.data === "string" ? ev.data : new Uint8Array(ev.data as ArrayBuffer));
  ws.onclose = (ev) => push({ closed: [ev.code, ev.reason] });
  const next = () => events.length ? Promise.resolve(events.shift()!) : new Promise<unknown>((r) => waiters.push(r));
  const opened = new Promise<void>((res, rej) => { ws.onopen = () => res(); ws.onerror = (e) => rej(e); });
...
test("http path still proxies and forwards the original host", async () => {
  ...
  const p = await fetch(`http://127.0.0.1:${cp.port}/post`, { method: "POST", headers: headers(), body: "body!" });
...
test("websocket: upstream refusal is passed through as a plain response", async () => {
  const r = await fetch(`http://127.0.0.1:${cp.port}/refuse`, {
    headers: { ...headers(), upgrade: "websocket", connection: "Upgrade", "sec-websocket-key": "x", "sec-websocket-version": "13" },
  });
...
test("websocket: no cookie -> 401 before dialing", async () => {
  ...
  const r = await fetch(`http://127.0.0.1:${cp.port}/ws`, { headers: { host: previewHost(), upgrade: "websocket", connection: "Upgrade" } });
