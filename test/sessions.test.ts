...
import { PresetRegistry, loadPresetDir } from "../src/cp/presets/index.ts";
...
function setup(maxServices = 2) {
  ...
  const cfg = { defaultImage: "default:latest", publicUrl: "https://cp.example.com", previewDomain: "preview.example.com", maxServices };
  const presets = new PresetRegistry(loaded.presets);
  return { store, svc: new SessionService(cfg, store, new NoHosts(), sched, new Tokens("k"), presets, loaded.catalog) };
}
...
test("create: preset picked by repo, caller values win, secrets never stored", () => {
  ...
  expect(v.image).toBe("devagents-coding-agent:latest");
...
test("create: preset defaults for image/cmd/idle apply unless the caller overrides; custom falls back to the CP default image", () => {
  ...
  expect(j.image).toBe("devagents-jupyter:latest");
...
test("services: inline declarations validated, secrets split by name, non-secret half visible in the view", () => {
  ...
  expect(v.services).toEqual([
    { name: "db", image: "postgres:16", env: { POSTGRES_DB: "app" }, cmd: null, ready: { port: 5432, timeout_s: 60 } },
    { name: "cache", image: "redis:7", env: {}, cmd: ["redis-server", "--appendonly", "yes"], ready: null },
  ]);
...
test("services: catalog names bring their sandbox env; caller env wins; references can rename and override", () => {
  const { svc } = setup();
  const v = svc.create({ owner_id: "me", repo: "https://x/r.git", services: ["postgres"], env: { PGDATABASE: "mine" } });
  expect(v.services.map((s) => [s.name, s.image, s.ready])).toEqual([["postgres", "postgres:16-alpine", { port: 5432, timeout_s: 90 }]]);
  expect(v.env).toMatchObject({ REPO: "https://x/r.git", DATABASE_URL: "postgres://devagents:devagents@postgres:5432/app", PGHOST: "postgres", PGDATABASE: "mine" });
  const w = svc.create({ owner_id: "me", services: [{ use: "redis", name: "cache", env: { X: "1" } }] });
  expect(w.services[0]).toMatchObject({ name: "cache", image: "redis:7-alpine", env: { X: "1" } });
  expect(w.env.REDIS_URL).toBe("redis://redis:6379"); // literal from the catalog: renaming does not rewrite URLs
  expect(() => svc.create({ owner_id: "me", services: ["mysql"] })).toThrow(/unknown service "mysql" \(catalog: postgres, redis\)/);
  expect(() => svc.create({ owner_id: "me", services: [{ use: "redis", name: "Bad" }] })).toThrow(/name must match/);
});
...
`;
  ...
  expect(v.services[1]).toEqual({ name: "db", image: "postgres:16", env: { POSTGRES_PASSWORD: "pw" }, cmd: ["postgres", "-c", "log_statement=all"], ready: { port: 5432, timeout_s: 60 } });
  expect(svc.create({ owner_id: "me", compose: { services: { db: { image: "i" } } } }).services).toHaveLength(1);
  expect(() => svc.create({ owner_id: "me", services: ["postgres"], compose: { services: { postgres: { image: "i" } } } })).toThrow(/name "postgres" is duplicated/);
  expect(() => svc.create({ owner_id: "me", services: ["postgres", "redis"], compose: { services: { a: { image: "i" }, b: { image: "i" } } } })).toThrow(/at most 3/);
  expect(() => svc.create({ owner_id: "me", compose: { services: { a: { image: "i", privileged: true } } } })).toThrow(/privileged is not supported/);
  expect(() => svc.create({ owner_id: "me", compose: 5 })).toThrow(/must be a compose document/);
});
