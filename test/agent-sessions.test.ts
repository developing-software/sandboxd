...
import type { CreateOpts, Duplex, PtyStream, SandboxDriver } from "../src/agent/driver.ts";
import type { EndReason, SessionSpec } from "../src/protocol/messages.ts";
...
class FakeDriver implements SandboxDriver {
  ...
  async attach(id: string, cmd: string[], env: Record<string, string>): Promise<PtyStream> {
    this.log.push(`attach ${id} ${cmd.join(" ")} ${Object.keys(env).sort().join(",")}`);
  ...
  async destroy(id: string) { this.log.push(`destroy ${id}`); }
  async createNetwork(sid: string) { this.log.push(`net+ ${sid}`); return `devagents-${sid}`; }
  async removeNetwork(sid: string) { this.log.push(`net- ${sid}`); }
  async listManaged() { return { containers: [], networks: [] }; }
}

const spec = (services: SessionSpec["services"]): SessionSpec => ({
  sid: "s_1", image: "sandbox:1", cmd: null, idle_timeout_s: 60, env: { A: "1" }, secret_env: { S: "x" }, services,
});
const services: SessionSpec["services"] = [
  { name: "db", image: "postgres:16", env: { POSTGRES_USER: "app" }, secret_env: { POSTGRES_PASSWORD: "pw" }, cmd: null, ready: { port: 5432, timeout_s: 5 } },
...
function setup() {
  ...
  const events: string[] = [];
  mgr.on({ started: (sid) => events.push(`started ${sid}`), ended: (sid, r: EndReason, d) => events.push(`ended ${sid} ${r}${d ? " " + d : ""}`) });
...
test("pod: network, services in order with readiness, then the sandbox; teardown in reverse", async () => {
  ...
  expect(driver.log).toEqual([
    "net+ s_1", "create c1:db", "dial c1:db:5432", "create c2:cache", "create c3:sandbox",
    "attach c3:sandbox /entry A,DEVAGENTS_SESSION_ID,S,TERM",
  ]);
  expect(driver.dials).toBe(3);
  const db = driver.created[0]!;
  expect([db.role, db.network, db.alias, db.env]).toEqual(["service", "devagents-s_1", "db", { POSTGRES_USER: "app", POSTGRES_PASSWORD: "pw" }]);
  expect(driver.created[2]).toEqual({ sid: "s_1", image: "sandbox:1", role: "sandbox", network: "devagents-s_1", alias: "sandbox" });
  ...
  expect(driver.log).toEqual(["destroy c3:sandbox", "destroy c2:cache", "destroy c1:db", "net- s_1"]);
...
test("a failing service image fails the session; earlier services and the network are cleaned up", async () => {
  ...
  expect(driver.log).toEqual(["net+ s_1", "create c1:db", "dial c1:db:5432", "destroy c1:db", "net- s_1"]);
});

test("a service's cmd (compose `command:`) reaches the driver; services without one get none", async () => {
  const { driver, mgr } = setup();
  await mgr.create(spec([
    { ...services[1]!, cmd: ["redis-server", "--appendonly", "yes"] },
    services[0]!,
  ]));
