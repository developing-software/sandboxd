...
test("dial failure that also fires onClose -> 502, no unhandled rejection", async () => {
  ...
  const h = (e: unknown) => { unhandled = e; };
  process.on("unhandledRejection", h);
  try {
    const res = await proxyOnce(req(), 3000, async (sub) => { sub.onClose(); throw new Error("connection refused"); });
...
test("normal response is streamed through", async () => {
  const res = await proxyOnce(req(), 3000, async (sub) => ({
    write() { queueMicrotask(() => { sub.onData(new TextEncoder().encode("HTTP/1.1 200 OK\r\ncontent-type: text/plain\r\ncontent-length: 2\r\n\r\nhi")); }); },
    close() {},
  }));
