...
import { interpolate, parseCompose, splitWords } from "../src/cp/compose.ts";
...
test("compose: anchors and merge keys, build-only app skipped, dependencies first, laptop keys ignored", () => {
  ...
  expect(db).toEqual({ name: "db", image: "postgres:16", env: { POSTGRES_PASSWORD: "pw", POSTGRES_PORT: "5432" }, cmd: null, ready: { port: 5432, timeout_s: 60 } });
  expect(cache).toEqual({ name: "cache", image: "redis:7", env: { A: "1", B: "two", C: "" }, cmd: ["redis-server", "--appendonly", "yes", "--requirepass", "a b"], ready: null });
...
test("compose: readiness port from x-devagents, expose, or ports (short, long, udp ignored)", () => {
  const ready = (svc: Record<string, unknown>) => parseCompose({ services: { s: { image: "i", ...svc } } }).decls[0]!.ready;
  expect(ready({ "x-devagents": { ready: { port: 9, timeout_s: 5 } }, expose: ["1"] })).toEqual({ port: 9, timeout_s: 5 });
  ...
  expect(ready({ ports: [{ target: 5432, published: 5433 }] })).toEqual({ port: 5432, timeout_s: 60 });
  expect(ready({ ports: [{ target: 53, protocol: "udp" }] })).toBeNull();
  expect(ready({})).toBeNull();
  expect(() => ready({ expose: ["a-b"] })).toThrow(/cannot read a port/);
  expect(() => ready({ "x-devagents": { ready: { port: 0 } } })).toThrow(/ready.port must be/);
});

test("compose: command forms, x-devagents secret_env / sandbox_env, interpolation", () => {
  const r = parseCompose({ services: {
    a: { image: "i:${TAG:-1}", command: ["sh", "-c", "echo $$HOME"], "x-devagents": { secret_env: { PW: "s" }, sandbox_env: { URL: "u://a" } } },
    b: { image: "j", command: "" },
  } });
...
test("compose: refused keys, bad shapes, cycles, names, empty and invalid input", () => {
  const svc = (s: Record<string, unknown>, name = "s") => () => parseCompose({ services: { [name]: { image: "i", ...s } } });
  ...
  expect(svc({ "x-devagents": { nope: 1 } })).toThrow(/x-devagents.nope is not supported/);
  ...
  expect(() => parseCompose({ services: { a: { image: "i", depends_on: ["b"] }, b: { image: "i", depends_on: ["a"] } } })).toThrow(/depends_on cycle: a -> b -> a/);
  expect(parseCompose({ services: { a: { image: "i", depends_on: ["gone", "b"] }, b: { image: "i" }, gone: { build: "." } } }).decls.map((d) => d.name)).toEqual(["b", "a"]);
  for (const empty of [undefined, null, "", "   ", {}, { services: null }]) expect(parseCompose(empty).decls).toEqual([]);
