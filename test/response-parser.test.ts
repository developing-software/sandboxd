...
function collect(p: ResponseParser, chunks: string[]) {
  let head: any = null, body = "", ended = false;
  for (const c of chunks) for (const ev of p.feed(b(c))) {
    if (ev.kind === "head") head = ev; else if (ev.kind === "body") body += s(ev.data); else ended = true;
  }
...
test("zero-length body ends immediately", () => {
  const r = collect(new ResponseParser(), ["HTTP/1.1 204 No Content\r\ncontent-length: 0\r\n\r\n"]);
