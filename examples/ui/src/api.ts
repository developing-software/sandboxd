...
import { type PreviewBody, Sandboxd } from '@sandboxd/sdk'
import { type Context, Hono } from 'hono'
import type { ContentfulStatusCode } from 'hono/utils/http-status'
import { type ApproveBody, type ClientOptions, Fleet } from './admin'
...
export function createApi(d: ApiDeps) {
  const { sandboxd, fleet } = clients(d)
  const reply = relay(d.cfg.apiUrl)

  return new Hono({ strict: false })
    .basePath('/api')
    .get('/presets', (c) => c.json(d.presets.list()))
    .get('/hosts', (c) => reply(c, fleet.listHosts()))
    .post('/hosts/:id/approve', async (c) =>
      reply(c, fleet.approveHost({ path: id(c), body: await json<ApproveBody>(c) })),
    )
    .post('/hosts/:id/revoke', (c) => reply(c, fleet.revokeHost({ path: id(c) })))
    .get('/sandboxes', (c) => reply(c, sandboxd.listSandboxes({ headers: owner(c) })))
    .post('/sandboxes', async (c) => {
      ...
      return reply(c, sandboxd.createSandbox({ headers: owner(c), body }))
    })
    .get('/sandboxes/:id', (c) =>
      reply(c, sandboxd.getSandbox({ path: id(c), headers: owner(c) })),
    )
    .delete('/sandboxes/:id', (c) =>
      reply(c, sandboxd.endSandbox({ path: id(c), headers: owner(c) })),
    )
    .post('/sandboxes/:id/terminal', (c) =>
      reply(c, sandboxd.openTerminal({ path: id(c), headers: owner(c) })),
    )
    .post('/sandboxes/:id/preview', async (c) =>
      reply(
        c,
        sandboxd.openPreview({
          path: id(c),
          headers: owner(c),
...
/**
 * Two SDKs on one listener, because there are two documents. They carry the same token
...
function clients(d: ApiDeps) {
  ...
  return {
    sandboxd: new Sandboxd({ ...config, serviceToken: d.cfg.serviceToken }),
    fleet: new Fleet({
      client: createClient(
