...
# TERM and DEVAGENTS_SESSION_ID. Everything else is plain env (see preset.yaml):
...
# Operators set defaults for any of these with DEVAGENTS_SANDBOX_ENV_<NAME> on the control plane.
...
# a caller's value or an operator's DEVAGENTS_SANDBOX_ENV_* always wins.
if [ -z "${AGENT:-}" ]; then
...
run_agent() {
  case "$AGENT" in
    ...
    codex)
      setup_codex
      case "$MODEL" in claude*) echo "⚠ codex with a Claude model: codex uses the Responses API, and LiteLLM's Responses→Bedrock translation is known to fail (tool_choice conflict). Prefer an OpenAI model for codex (set CODEX_MODEL, e.g. DEVAGENTS_SANDBOX_ENV_CODEX_MODEL on the control plane)." ;; esac
