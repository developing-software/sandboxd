...
export class Tunnel {
  ...
  constructor(private cfg: AgentConfig, private sessions: SessionManager, private driver: SandboxDriver) {}
  ...
  private startHeartbeat() {
    ...
    this.heartbeat = setInterval(() => {
      this.send({ type: "heartbeat", running: this.sessions.count(), max: this.cfg.maxSessions });
    }, 10_000);
  }
  ...
  private onControl(msg: CpMsg) {
    switch (msg.type) {
      ...
      case "pty.open": {
        ...
        const attached = this.sessions.attachViewer(msg.sid, msg.size, (d) => this.sendBinary(stream, d));
        if (!attached) { this.send({ type: "pty.closed", stream }); return; }
        this.streams.set(stream, { kind: "pty", sid: msg.sid, unsubscribe: attached.unsubscribe });
  ...
  private async dial(sid: string, port: number, stream: number) {
    ...
    if (!containerId) { this.send({ type: "port.error", stream, msg: "no such session" }); return; }
    try {
