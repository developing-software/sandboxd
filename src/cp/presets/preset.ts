...
import { resolveRef, noServices, type ServiceCatalog, type ValidatedServices } from "../services.ts";
import type { PresetDoc } from "./schema.ts";
import type { Body, Expanded, FieldSpec, Preset } from "./types.ts";

export function presetFromDoc(doc: PresetDoc, catalog: ServiceCatalog): Preset {
  for (const r of doc.serviceRefs) {
    if (!catalog.get(r.use)) throw new Error(`services: unknown catalog service "${r.use}"${catalog.names.length ? ` (catalog: ${catalog.names.join(", ")})` : " (no presets/services.yaml)"}`);
  }
  const { claims, serviceRefs, ...info } = doc;
  return {
    name: doc.name,
    info,
    ...(claims.length ? { claims: (body: Body) => claims.every((c) => present(readPath(body, c))) } : {}),
...
function resolveAll(refs: PresetDoc["serviceRefs"], catalog: ServiceCatalog): ValidatedServices {
...
function fieldValue(f: FieldSpec, raw: unknown): string | undefined {
  ...
  switch (f.type) {
    case "string":
      ...
      return v;
    case "enum":
      if (typeof v !== "string" || !f.values!.includes(v)) throw badRequest(`${f.name} must be one of ${f.values!.join(", ")}`);
      return v;
    case "url":
      if (typeof v !== "string" || !/^https?:\/\//.test(v)) throw badRequest(`${f.name} must be http(s)`);
      return v.replace(/\/+$/, "");
    case "int": {
      const lo = f.min ?? Number.MIN_SAFE_INTEGER, hi = f.max ?? Number.MAX_SAFE_INTEGER;
      if (!Number.isInteger(v) || (v as number) < lo || (v as number) > hi) {
        throw badRequest(`${f.name} must be an integer${f.min !== undefined || f.max !== undefined ? ` in ${f.min ?? ""}..${f.max ?? ""}` : ""}`);
      }
      return String(v);
    }
    case "bool":
...
function readPath(body: Body, path: string): unknown {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if (typeof cur !== "object" || Array.isArray(cur)) throw badRequest(`${parts.slice(0, i).join(".")} must be an object`);
