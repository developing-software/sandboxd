...
export interface TokenPayload { k: TokenKind; sid: string; port?: number; exp: number }
...
export class Tokens {
  ...
  sign(p: Omit<TokenPayload, "exp">, ttlMs: number): string {
    const body = Buffer.from(JSON.stringify({ ...p, exp: Date.now() + ttlMs })).toString("base64url");
