...
import type { HostMsg } from "../../protocol/messages.ts";
...
export function enroll(store: Store, hello: Hello): Enrollment {
  ...
  if (!host) {
    host = store.insertPendingHost({
      id: newHostId(), name: hello.name, fingerprint: hello.fingerprint,
      approve_code: newApproveCode(), max_sessions: hello.max_sessions,
    });
  }
  store.touchHost(host.id, { name: hello.name, max_sessions: hello.max_sessions });
  switch (host.status) {
    case "revoked": return { kind: "rejected", host, reason: "revoked" };
    case "pending": return { kind: "pending", host, code: host.approve_code!, isNew };
    case "approved": return { kind: "accepted", host };
  }
}
