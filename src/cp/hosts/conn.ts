...
import type { CpMsg, HostMsg, SessionSpec, Size } from "../../protocol/messages.ts";
...
export interface Capacity { running: number; max: number }

/** What a HostConn needs from its socket. Bun's ServerWebSocket satisfies it; tests pass a stub. */
export interface Transport { send(data: string | Uint8Array): unknown; close(): void }
...
export class HostConn {
  ...
  constructor(readonly hostId: string, readonly transport: Transport, running: number, max: number) {
    this.running = running;
    this.max = max;
  }

  capacity(): Capacity { return { running: this.running, max: this.max }; }
  send(msg: CpMsg) { this.transport.send(JSON.stringify(msg)); }
  close() { this.transport.close(); }

  // ---- inbound from the host ----
  onFrame(stream: number, payload: Uint8Array) { this.streams.get(stream)?.onData(payload); }

  onStreamMsg(msg: StreamMsg) {
    switch (msg.type) {
      case "pty.replay": this.streams.get(msg.stream)?.onReplay?.(); return;
      case "port.open": this.streams.get(msg.stream)?.onOpen?.(); return;
      case "port.error": this.streams.get(msg.stream)?.onError?.(msg.msg); this.endStream(msg.stream); return;
      case "pty.closed":
      case "port.close": this.endStream(msg.stream); return;
    }
  }
  ...
  destroySession(sid: string) { this.send({ type: "session.destroy", sid }); }
  ...
  openPty(sid: string, size: Size, sub: StreamSub): PtyHandle {
    ...
    return {
      ...
      close: () => { if (this.streams.delete(stream)) this.send({ type: "pty.close", stream }); },
    };
  }

  /** Resolves on `port.open`, rejects on `port.error`. `sub.onClose` fires in both the error and the normal-close case. */
  dialPort(sid: string, port: number, sub: Pick<StreamSub, "onData" | "onClose">): Promise<PortHandle> {
    const { promise, resolve, reject } = Promise.withResolvers<PortHandle>();
    let stream = 0;
    const handle: PortHandle = {
      write: (d) => this.transport.send(encodeFrame(stream, d)),
      close: () => { if (this.streams.delete(stream)) this.send({ type: "port.close", stream }); },
    };
