...
import { noServices, validateName, validateReady, type ValidatedServices } from "./services.ts";

/** Keys the translation acts on. */
const HANDLED = new Set(["image", "build", "environment", "command", "depends_on", "expose", "ports", "x-devagents"]);
/** Keys that describe a laptop setup (host ports, bind mounts, restart policy…) and are safely dropped. */
const IGNORED = new Set([
  "volumes", "restart", "container_name", "networks", "profiles", "deploy", "logging", "labels", "healthcheck",
  "stdin_open", "tty", "pull_policy", "platform", "env_file", "hostname", "working_dir", "init", "develop", "attach",
  "stop_grace_period", "stop_signal", "annotations", "scale", "links", "external_links",
]);
/** Keys that would change what runs on the host. Refused so the caller fixes the file rather than getting a surprise. */
const REJECTED = new Set([
  "entrypoint", "privileged", "cap_add", "cap_drop", "devices", "network_mode", "pid", "ipc", "uts", "user", "userns_mode",
  "extra_hosts", "sysctls", "security_opt", "cgroup", "cgroup_parent", "volumes_from", "extends", "tmpfs", "read_only",
  "runtime", "isolation", "group_add", "dns", "dns_search", "dns_opt", "mac_address", "domainname", "shm_size", "ulimits",
  "oom_kill_disable", "oom_score_adj", "storage_opt", "device_cgroup_rules", "credential_spec", "configs", "secrets",
  "gpus", "models", "provider", "post_start", "pre_stop", "label_file", "blkio_config", "mem_limit", "mem_reservation",
  "mem_swappiness", "memswap_limit", "cpus", "cpu_count", "cpu_percent", "cpu_shares", "cpu_period", "cpu_quota",
  "cpu_rt_runtime", "cpu_rt_period", "cpuset", "pids_limit",
]);
...
export function parseCompose(input: unknown, at = "compose"): ValidatedServices {
  ...
  if (typeof input === "string") {
    ...
    try { doc = Bun.YAML.parse(input); } catch (e) { throw badRequest(`${at}: invalid YAML: ${(e as Error).message}`); }
  }
  ...
  for (const [key, raw] of Object.entries(services)) {
    ...
    for (const k of Object.keys(raw)) {
      ...
      throw badRequest(`${sat}.${k} is not supported${REJECTED.has(k) ? "" : " (unknown key)"}`);
    }
    if (typeof raw.image !== "string" || !raw.image.trim()) throw badRequest(`${sat}.image is required`);
    const x = raw["x-devagents"] === undefined ? {} : raw["x-devagents"];
    if (!isObj(x)) throw badRequest(`${sat}.x-devagents must be a map`);
    for (const k of Object.keys(x)) if (!X_KEYS.has(k)) throw badRequest(`${sat}.x-devagents.${k} is not supported`);

    const decl: ServiceDecl = {
      name,
      image: interpolate(raw.image.trim(), `${sat}.image`),
      env: validateEnv(`${sat}.environment`, environment(raw.environment, `${sat}.environment`)),
      cmd: command(raw.command, `${sat}.command`),
      ready: x.ready !== undefined ? validateReady(`${sat}.x-devagents`, x.ready) : readyFromPorts(raw, sat),
    };
    if (out.decls.some((d) => d.name === name)) throw badRequest(`${sat}: duplicated`);
    out.decls.push(decl);
    const secret = validateEnv(`${sat}.x-devagents.secret_env`, x.secret_env);
    if (Object.keys(secret).length) out.secrets[name] = secret;
    const sandbox = validateEnv(`${sat}.x-devagents.sandbox_env`, x.sandbox_env);
...
function environment(raw: unknown, at: string): Record<string, string> {
  ...
  if (Array.isArray(raw)) {
    for (const item of raw) {
      ...
      if (i < 0) { out[item] = ""; continue; } // bare KEY takes the value from the host shell in compose; there is none here
...
function command(raw: unknown, at: string): string[] | null {
  ...
  if (Array.isArray(raw) && raw.length && raw.every((c) => typeof c === "string" || typeof c === "number")) return raw.map((c) => interpolate(String(c), at));
  throw badRequest(`${at} must be a string or a list of strings`);
}

/** Readiness port when `x-devagents.ready` is absent: first `expose` entry, else the container side of the first `ports` entry. */
function readyFromPorts(raw: Record<string, unknown>, at: string): ServiceDecl["ready"] {
  ...
  const p = isObj(port) ? containerPort(String(port.target ?? ""), String(port.protocol ?? "tcp")) : containerPort(String(port));
...
function dependsOn(raw: unknown, at: string): string[] {
  ...
  if (Array.isArray(raw)) {
    if (!raw.every((d) => typeof d === "string")) throw badRequest(`${at} entries must be service names`);
...
function startOrder(decls: ServiceDecl[], deps: Map<string, string[]>, at: string): ServiceDecl[] {
  ...
  const visit = (name: string) => {
    ...
    if (i >= 0) throw badRequest(`${at}: depends_on cycle: ${[...stack.slice(i), name].join(" -> ")}`);
    stack.push(name);
...
export function interpolate(s: string, at: string): string {
  ...
  return s.replace(/\$(?:(\$)|\{([^}]*)\}|([A-Za-z_][A-Za-z0-9_]*))/g, (m, dollar, braced, bare) => {
    if (dollar) return "$";
    ...
    switch (kind) {
      ...
      case "+": case ":+": return "";
      default: throw badRequest(`${at}: ${arg || `variable ${name} is required`} (set it with \${${name}:-default} — the control plane has no environment to read it from)`);
    }
  });
}
