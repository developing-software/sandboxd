...
export const noServices = (): ValidatedServices => ({ decls: [], secrets: {}, sandbox_env: {} });

/** A catalog entry: a validated service the caller can pick by name. */
export interface CatalogEntry { decl: ServiceDecl; secret_env: Record<string, string>; sandbox_env: Record<string, string> }
export interface ServiceCatalog { readonly names: string[]; get(name: string): CatalogEntry | undefined }
export const EMPTY_CATALOG: ServiceCatalog = { names: [], get: () => undefined };

/** `services: ["postgres"]` or `{use: "postgres", name: "db", env: {...}}`. */
export interface ServiceRef { use: string; name?: string; env?: Record<string, string>; secret_env?: Record<string, string> }

export interface ServiceInput {
  name?: unknown; image?: unknown; env?: unknown; secret_env?: unknown; ready?: unknown; cmd?: unknown; use?: unknown;
}

/** The caller's `services` array: inline declarations and catalog references, in order. */
export function validateServices(raw: unknown, catalog: ServiceCatalog = EMPTY_CATALOG): ValidatedServices {
  ...
  raw.forEach((item: unknown, i) => {
    ...
    if (!item || typeof item !== "object" || Array.isArray(item)) throw badRequest(`${at} must be an object`);
    const s = item as ServiceInput;
    if (s.use !== undefined) {
      if (typeof s.use !== "string") throw badRequest(`${at}.use must be a string`);
      if (s.name !== undefined && typeof s.name !== "string") throw badRequest(`${at}.name must be a string`);
      return add(out, resolveRef({ use: s.use, name: s.name as string | undefined, env: validateEnv(`${at}.env`, s.env), secret_env: validateEnv(`${at}.secret_env`, s.secret_env) }, catalog, at), at);
    }
    const name = validateName(at, s.name);
    if (typeof s.image !== "string" || !s.image.trim()) throw badRequest(`${at}.image is required`);
    const decl: ServiceDecl = { name, image: s.image.trim(), env: validateEnv(`${at}.env`, s.env), cmd: validateCmd(at, s.cmd), ready: validateReady(at, s.ready) };
    add(out, { decl, secret_env: validateEnv(`${at}.secret_env`, s.secret_env), sandbox_env: {} }, at);
  });
  return out;
}

/** A catalog entry with the reference's overrides applied. */
export function resolveRef(ref: ServiceRef, catalog: ServiceCatalog, at: string): CatalogEntry {
  const entry = catalog.get(ref.use);
  if (!entry) throw badRequest(`${at}: unknown service "${ref.use}"${catalog.names.length ? ` (catalog: ${catalog.names.join(", ")})` : " (the catalog is empty)"}`);
...
export function mergeServices(parts: ValidatedServices[], max: number): ValidatedServices {
  ...
  for (const p of parts) {
    for (const d of p.decls) {
      if (out.decls.some((x) => x.name === d.name)) throw badRequest(`services: name "${d.name}" is duplicated`);
      out.decls.push(d);
...
function add(out: ValidatedServices, e: CatalogEntry, at: string) {
  if (out.decls.some((x) => x.name === e.decl.name)) throw badRequest(`${at}.name "${e.decl.name}" is duplicated`);
...
export function validateName(at: string, name: unknown): string {
  if (typeof name !== "string" || !NAME_RE.test(name)) throw badRequest(`${at}.name must match ${NAME_RE}`);
...
export function validateCmd(at: string, cmd: unknown): string[] | null {
  ...
  if (!Array.isArray(cmd) || cmd.length === 0 || !cmd.every((c) => typeof c === "string")) throw badRequest(`${at}.cmd must be a non-empty array of strings`);
...
export function validateReady(at: string, ready: unknown): ServiceDecl["ready"] {
  ...
  if (typeof ready !== "object" || Array.isArray(ready)) throw badRequest(`${at}.ready must be an object`);
  const { port, timeout_s } = ready as { port?: unknown; timeout_s?: unknown };
  if (!Number.isInteger(port) || (port as number) < 1 || (port as number) > 65535) throw badRequest(`${at}.ready.port must be 1-65535`);
  const t = timeout_s ?? DEFAULT_READY_TIMEOUT_S;
  if (!Number.isInteger(t) || (t as number) < 1 || (t as number) > MAX_READY_TIMEOUT_S) throw badRequest(`${at}.ready.timeout_s must be 1-${MAX_READY_TIMEOUT_S}`);
