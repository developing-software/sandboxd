...
import type { PortDialer } from "../hosts/hub.ts";
import type { Store } from "../store.ts";
import { Tokens } from "../tokens.ts";
import { COOKIE, proxyOnce, type Dial } from "./http.ts";
import { proxyWebSocket, type PreviewUpgrader } from "./ws-bridge.ts";

export { COOKIE } from "./http.ts";
export type { PreviewWsData, PreviewUpgrader } from "./ws-bridge.ts";

const COOKIE_TTL_MS = 12 * 60 * 60 * 1000;

export interface PreviewTarget { port: number; sid: string }

export class PreviewProxy {
  private hostRe: RegExp;

  constructor(private store: Store, private hub: PortDialer, private tokens: Tokens, private previewDomain: string) {
  ...
  async handle(req: Request, { port, sid }: PreviewTarget, server: PreviewUpgrader): Promise<Response | undefined> {
